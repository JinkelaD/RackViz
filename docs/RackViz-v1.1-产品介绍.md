...
- 🐛 **修复下架失效 P0**：COALESCE 无法置空 → 三态 Patch 更新（拖回资源池/未上架真正生效）
- 🔒 **安全加固**：CSP 策略启用、LIKE 通配符转义、日志敏感信息脱敏、权限面收紧
- 🗃️ **数据库 v3**：7 个索引 + UNIQUE 约束 + 迁移事务保护（老库自动升级）
- ⚡ **性能**：导出异步化（UI 不再冻结）、N+1 查询消除、前端 Map 化稳定引用
- 🧹 **代码质量**：RackView/DeviceList 组件化拆分（< 300 行）、LayoutContext 拆分为小 Context、
  类型单一来源、Clippy/ESLint/离线红线检查、版本号统一 1.2.0
- 📤 **导出接线**：机柜部署图 / 单机柜导出（此前已实现但无 UI 入口）
