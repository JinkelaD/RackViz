...
pub fn export_racks_excel(conn: &Connection) -> Result<Vec<u8>, AppError> {
    ...
    let mut col = 1u16;
    for rack in &racks {
        sheet.write_string_with_format(0, col, &rack.name, &header_fmt)?;
        col += 1;
    }
    ...
    for (idx, rack) in racks.iter().enumerate() {
        ...
        let devices = db::devices::list_devices(conn, Some(rack.id), None)?;
        for dev in &devices {
...
pub fn import_devices_excel(conn: &Connection, file_path: &str) -> Result<ImportResult, AppError> {
    ...
    db::with_transaction(conn, |c| {
        for (row_idx, row) in range.rows().enumerate() {
            ...
            let name = cell_to_string(row.get(0).unwrap_or(&calamine::Data::Empty)).trim().to_string();
            ...
            if !serial_no.is_empty() {
                if let Some(existing) = db::devices::find_device_by_serial(c, &serial_no)? {
                    log::warn!("导入跳过第{}行: 序列号 {} 已被设备「{}」使用", row_idx + 1, serial_no, existing.name);
                    skipped += 1;
                    continue;
                }
            }
            ...
            if !asset_no.is_empty() {
                if let Some(existing) = db::devices::find_device_by_asset(c, &asset_no)? {
                    log::warn!("导入跳过第{}行: 资产编号 {} 已被设备「{}」使用", row_idx + 1, asset_no, existing.name);
                    skipped += 1;
                    continue;
                }
            }
            ...
            // 3. 同机柜内名称+位置冲突检查
            if let Some(_existing) = db::devices::find_device_by_name_in_rack(c, &name, rack_id)? {
                log::warn!("导入跳过第{}行: 机柜中已存在同名设备「{}」", row_idx + 1, name);
                skipped += 1;
                continue;
            }
